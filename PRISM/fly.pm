...
module flystate
	...
	[] (z=true & s=0) -> 0.5 : (s'=1) + 0.5 : (s'=2);

	// If Cast, MC walk over directions
	[] s=1 -> 1-a : (s'=1) + a : (s'=2);
	[] s=2 -> a : (s'=1) + 1-a : (s'=2);

	// If outside arena, return after exp. dist. time
	[] s=1 & x<=-ext -> 1.0 : (s'=3);
	[] s=3 & x<=-ext -> d: (s'=2) & (x'=-ext+1) + 1-d : (s'=s);
	[] s=2 & x>=ext -> 1.0 : (s'=3);
	[] s=3 & x<=-ext -> d: (s'=1) & (x'=ext-1) + 1-d : (s'=s);

	// Change state based on stochastic plume sensing
	[] (s=1 | s=2) & (x>-plume & x<plume) -> b : (s'=4) + 1-b : (s'=s);
	[] (s=1 | s=2) & !(x>-plume & x<plume) -> c : (s'=4) + 1-c : (s'=s);
	[] s=4 & !(x>-plume & x<plume) -> b: (s'=0) + 1-b : (s'=s);
	[] s=4 & (x>-plume & x<plume) -> c: (s'=0) + 1-c : (s'=s);

	// If near target, success
	[] (s=1 | s=2 | s=4) & (pow(x,2)+pow(y,2)<pow(10,2)) -> 1.0 : (s'=6);
